...
echo "[0] NOVA,HE — FULL Flask Self-Healing + Load Balancer Auto Diagnostic"
...
if [[ "$LB_STATUS" == "HEALTHY" ]]; then
    ...
    echo "[2a] Restarting Flask app..."
    pkill -f ai_website_builder.py || true
    nohup python3 $FLASK_APP_PATH > /var/log/flask_ai.log 2>&1 &
    sleep 10

    echo "[2b] Verifying firewall rules..."
    gcloud compute firewall-rules create allow-http --allow tcp:80 --direction=INGRESS --target-tags=http-server --source-ranges=0.0.0.0/0 --project=$PROJECT_ID --quiet || true

    echo "[2c] Retesting backend health..."
    LB_STATUS=$(gcloud compute backend-services get-health $BACKEND_SERVICE_NAME --global --project=$PROJECT_ID --format="value(status.healthStatus[0].healthState)")
    if [[ "$LB_STATUS" == "HEALTHY" ]]; then
