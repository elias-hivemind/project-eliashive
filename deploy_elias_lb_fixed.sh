...
echo "[0] EliasNorth Load Balancer Setup — FINAL CONFIG]"
...
echo "[1] Auth with Elias service account..."
...
echo "[3] Reserve Static IP..."
gcloud compute addresses create $STATIC_IP_NAME \
  --ip-version=IPV4 --global

echo "[4] Create Health Check..."
gcloud compute health-checks create http $HEALTH_CHECK_NAME \
  --port=80 --request-path=/ --global

echo "[5] Create Backend Service..."
gcloud compute backend-services create $BACKEND_SERVICE_NAME \
  --protocol=HTTP --health-checks=$HEALTH_CHECK_NAME --global

echo "[6] Attach Instance Group..."
gcloud compute backend-services add-backend $BACKEND_SERVICE_NAME \
  --instance-group=$INSTANCE_GROUP \
  --instance-group-zone=$ZONE \
  --global

echo "[7] Create URL Map..."
gcloud compute url-maps create $URL_MAP_NAME \
  --default-service=$BACKEND_SERVICE_NAME

echo "[8] Create SSL Certificate..."
gcloud compute ssl-certificates create $SSL_CERT_NAME \
  --domains=$DOMAIN --global

echo "[9] Create HTTPS Proxy..."
gcloud compute target-https-proxies create $HTTPS_PROXY_NAME \
  --ssl-certificates=$SSL_CERT_NAME \
  --url-map=$URL_MAP_NAME

echo "[10] Create Forwarding Rule..."
...
echo "[11] Fetch Reserved IP:"
gcloud compute addresses describe $STATIC_IP_NAME \
  --global --format="get(address)"

echo "[DONE] Point A record to the above IP in Namecheap."
